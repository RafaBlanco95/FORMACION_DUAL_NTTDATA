...
public class HibernateMain {
	...
	public static void main(String[] args) {
		...
		// Consumición del servicio de contratos para mostrar las personas que tinen un determinado
		// apellido y pagan mensualmente una determinada cantidad por el contrato.
		// Consulta realizada utilizando JPA Criteria.
		sbu.append("Método: main() | Llamada al método searchByLastNameAndContractCost() de contractService");
		LOG.info("{}", sbu);
		List<Contract> contractCriteria = contractService.searchByLastNameAndContractCost(Blanco, 500F);
		for (final Contract contract : contractCriteria) {
			sbu.append(contract.getCustomer().getName());
			sbu.append(" | ");
			sbu.append(contract.getCustomer().getFirstLastName());
			sbu.append(" | ");
			sbu.append(contract.getMonthlyCost());
			LOG.info("{}", sbu);
			sbu.setLength(0);
